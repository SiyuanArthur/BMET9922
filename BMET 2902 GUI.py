...
class App:  # 应用主类：负责窗口、图形、右侧面板、日志、与后台线程的协作
    def __init__(self, root: tk.Tk, backend_cmd: str | None = None):  # root 是 Tk 根窗口；backend_cmd 预留给外部数据源命令
        ...
        ttk.Button(btns, text="EXIT",
                   command=self.root.destroy).pack(side="left", padx=2)  # 退出程序
        ...
        self.root.after(UI_UPDATE_MS, self._poll_queue)  # 每 UI_UPDATE_MS 毫秒拉取队列数据并入缓冲
        self.root.after(UI_UPDATE_MS, self._redraw)  # 每 UI_UPDATE_MS 毫秒重绘图形/报警/数值卡片
    ...
    def _poll_queue(self):  # 从后台线程队列取消息，更新本地缓冲
        try:
            while True:  # 一次性把当前可取的消息全部取空（非阻塞）
                typ, payload = self.q.get_nowait()  # 队列空会抛 queue.Empty（被下方 except 捕获）

                if typ == "log":  # 后台发来的日志
                    self._log(payload)  # 直接写日志窗口

                elif typ == "bpm":  # 简包：只有 bpm（也用于模拟的高频点）
                    t, v = payload  # t=主机时间戳，v=bpm
                    self.last_pkt_ts = t  # 也视为“收到活动”以喂狗（模拟模式依然能清除看门狗）
                    self.ts.append(t); self.bpm.append(v)  # 追加到时间序列
                    cutoff = time.time() - HISTORY_SECONDS  # 裁剪阈值：只留最近 N 秒
                    while self.ts and self.ts[0] < cutoff:  # 从左侧弹出过期数据
                        self.ts.popleft(); self.bpm.popleft()

                elif typ == "pkt":  # 完整包：含 bpm + 50 个样本 + seq + flags + t_mcu
                    t_host, bpm, samples, seq, flags, t_mcu = payload  # 解包
                    self.last_pkt_ts = t_host  # 看门狗用“包到达”的主机时间计时
                    self.ts.append(t_host); self.bpm.append(bpm)  # BPM 时间线也更新

                    # Waveform (assume 50 Hz, 1-second chunk) —— 将 50 个样本展开到时间轴
                    base = t_host - 1.0  # 这 50 点代表“上一秒”的波形
                    for i, y in enumerate(samples):  # i=0..49
                        self.wave_t.append(base + i / 50.0)  # 以 20ms 间隔生成时间戳
                        self.wave_y.append(y)  # 追加样本值

                    # trim —— 裁剪波形与 BPM 缓冲，保持最近 N 秒窗口
            ...
            self.root.after(UI_UPDATE_MS, self._poll_queue)  # 预约下一次轮询，形成自驱动循环
    ...
    def _redraw(self):  # 重绘图形与报警指示
        ...
        if mode == "PULSE":  # —— 波形视图 —— #
            ...
            self.root.after(UI_UPDATE_MS, self._redraw)  # 预约下一次重绘
