...
class App:  # 应用主类：负责窗口、图形、右侧面板、日志、与后台线程的协作
    ...
    def _poll_queue(self):  # 从后台线程队列取消息，更新本地缓冲
        try:
            while not self.data_queue.empty():  # 一次性把当前可取的消息全部取空（非阻塞）
                line = self.data_queue.get() # 队列空会抛 queue.Empty（被下方 except 捕获）
            ...
            self._after_poll = self.root.after(UI_UPDATE_MS, self._poll_queue)# 预约下一次轮询，形成自驱动循环
